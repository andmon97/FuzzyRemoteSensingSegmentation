...
'''
    ...
    1) Trasformato il conseguente delle regole da regole TSK di ordine n a ordine 0;
    2) Normalizzato il valore dei conseguenti delle regole.
    
    Variabile introdotta nterms 
'''
...
class ConsequentLayer(torch.nn.Module):
    ...
    def fit_coeff(self, x, weights, y_actual):
        ...
        try:
            #lstqs usato nella nuova versione di Pytorch
            coeff_2d, _ = torch.lstsq(y_actual_2d, weighted_x_2d) #torch.gels(y_actual_2d, weighted_x_2d)
    ...
    def forward(self, x):
        ...
        y_pred = torch.matmul(self.coeff, x_plus.t()) #PRODOTTO DI MATRICE DA DUE TENSORI
